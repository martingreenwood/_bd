  ...
  // If post is 'dimmed'
	...
	if (is_archive()):
		...
		if (get_sub_field("thumbs_width")):
			...
			// Default Archives
			if (get_field('thumbs_width', $taxonomy . '_' . $term_id)):
			...
			if (get_field('thumbs_title_style', $taxonomy . '_' . $term_id)) {
				$thumbs_title_style = get_field('thumbs_title_style', $taxonomy . '_' . $term_id);
			} else {
				$thumbs_title_style = 'Overlay';
			}

			if (get_field('thumbs_overlay_title_position', $taxonomy . '_' . $term_id)) {
				$thumbnails_title_position = get_field('thumbs_overlay_title_position', $taxonomy . '_' . $term_id);
			} else {
				$thumbnails_title_position = 'Center';
			}

			if (get_field('thumbs_below_align', $taxonomy . '_' . $term_id)) {
				$thumbnails_title_align = get_field('thumbs_below_align', $taxonomy . '_' . $term_id);
			} else {
				$thumbnails_title_align = 'align-center';
			}

		endif; //thumbs_width
	
	endif; //end if is archive
	...
	if( has_post_thumbnail() ): ?>
		...
		<!-- Masonry Item -->
		<div class="masonry-item item thumb-item col col-<?php if ($thumbswidth) {echo $thumbswidth;} else {echo '25';} ?>">
			<div class="item-content col-inner" style="<?php if ($state) {echo 'margin: '.$override_grid_gutter.'px !important;';} ?>">
			...
			<a href="<?php the_permalink(); ?>" class="history thumb <?php if ($pagetype == 'single' || $pagetype == 'page') { if ($wp_query->post->ID == $postid || $wp_query->post->ID == $this_page_id) {echo 'active';} } ?> <?php if (get_field("advanced_thumbnail")) {echo 'advanced';} ?>" <?php if ($opacitycalc) {echo 'data-opacity="'.$opacitycalc.'"';} ?> <?php if ($color) {echo 'data-bg-color="'.$color.'"';} ?> <?php if ($textcolor) {echo 'data-text-color="'.$textcolor.'"';} ?> <?php if ($hovertextcolor) {echo 'data-hover-text-color="'.$hovertextcolor.'"';} ?>>
				...
				<div class="img-holder">
					...
					<?php if ($thumbs_title_style == 'Overlay') { ?>
					  <div class="figcaption <?php echo $thumbnails_title_text_size; ?>" style="<?php if ($textcolor) {echo 'color:'.$textcolor.';';} ?>">
					  <?php if ($thumbnails_title_position == 'Center') { ?>
						...
						<?php if (get_field('thumbnail_title')) { ?>
						  <div class="vcenter p0 <?php echo $thumbnails_title_position; ?>">
							<span class="underline" style="<?php if ($textcolor) {echo 'color:'.$textcolor.';';if ($link_decoration == '1px always' || $link_decoration == '1px on mouseover and marked') {echo 'border-bottom: 1px solid '.$textcolor.';';} else { echo 'border-bottom: 1px solid transparent'; }} ?>">
							  <?php echo get_field('thumbnail_title'); ?>
							</span>
						  </div>
						<?php } else { ?>
						  <div class="vcenter p0 <?php echo $thumbnails_title_position; ?>">
